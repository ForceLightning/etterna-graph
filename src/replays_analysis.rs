...
/// Rust struct which contains all relevant information regarding a player's replay stats.
#[pyclass]
#[derive(Default, Debug)]
pub struct ReplaysAnalysis {
    /// The indices of the scores which analysis didn't fail.
    #[pyo3(get)]
    pub score_indices: Vec<u64>,
    /// A Vec of percentages (from 0.0 to 1.0) that says how many notes were hit out of order.
    #[pyo3(get)]
    pub manipulations: Vec<f32>,
    /// The mean of all non-CB offsets.
    #[pyo3(get)]
    pub deviation_mean: f32,
    #[pyo3(get)]
    /// Number of notes per column.
    pub notes_per_column: [u64; 4],
    #[pyo3(get)]
    /// Number of combo-breaks per column.
    pub cbs_per_column: [u64; 4],
    #[pyo3(get)]
    /// Longest combo and associated scorekey
    pub longest_mcombo: (u64, String),
    #[pyo3(get)]
    /// A vector of size `NUM_OFFSET_BUCKETS`, aggregated over all `ScoreAnalysis.offset_buckets`.
    pub offset_buckets: Vec<u64>,
    /// A vector of size `NUM_OFFSET_BUCKETS`, aggregated over all `ScoreAnalysis.offset_buckets`
    /// if the `ScoreAnalysis.manipulation` < 0.93.
    #[pyo3(get)]
    ...
    /// Like `score_indices`, but just for the timing info dependent fields.
    #[pyo3(get)]
    pub timing_info_dependent_score_indices: Vec<u64>,
    #[pyo3(get)]
...
struct ScoreAnalysis {
    ...
    timing_info_dependent_analysis: Option<TimingInfoDependentAnalysis>,
}

/// This is the part of score analysis that's dependent on timing info and therefore may not be
/// available for every score
struct TimingInfoDependentAnalysis {
...
pub fn parse_sm_float(string: &[u8]) -> Option<f32> {
    ...
    lexical_core::parse::<f32>(string).ok()
...
/// Finds the fastest subset of notes between `min_num_notes` and `max_num_notes`.
///     NOTE: The caller still has to scale the returned nps by the music rate and `seconds` must be sorted.
fn find_fastest_note_subset(
    seconds: &[f32],
    min_num_notes: u64,
...
/// Finds the fastest subset of notes between `min_num_notes` and `max_num_notes` which is scaled
/// by the respective `wife_pts`.
///     NOTE: The caller still has to scale the returned nps by the music rate `seconds` must be sorted
///     and in the same order as `wife_pts`
fn find_fastest_note_subset_wife_pts(
    seconds: &[f32],
    min_num_notes: u64,
    max_num_notes: u64, // inclusive
    wife_pts: &[f32],
    ...
    for n in (min_num_notes as usize)..end_n {
        ...
        for i in 0..=(seconds.len() - n - 1) {
            ...
            if nps >= fastest.speed {
                // why >=? see other note subset function
                fastest = FastestComboInfo {
                    length: n as u64,
                    start_second: seconds[i],
                    end_second: seconds[end_i],
...
/// Finds the fastest combo for a score.
/// # Arguments
/// * `seconds` - Array of seconds in a chart region.
/// * `are_cbs` - Iterator of bools to signify if the combo is broken.
/// * `min_num_notes` - Minimum number of notes to consider.
/// * `max_num_notes` - Maximum number of notes to consider.
/// * `wife_pts` - If provided, the nps will be multiplied by wife pts. The 'nps' is practically
///     'wife points per second' then.
/// * `rate` - Rate for the score.
fn find_fastest_combo_in_score<I, T>(
    seconds: &[f32],
    are_cbs: I,
    min_num_notes: u64,
    max_num_notes: u64,
...
/// Creates a histogram of hit-timing deviations.
/// # Arguments
/// * `deviations` - A vec of hit-timing deviations.
///
/// # Returns
/// * Histogram of hit-timing deviations.
...
fn analyze(
    ...
    for (&_tick, &deviation, &column) in
        izip!(&unsorted_ticks, &unsorted_deviations, &unsorted_columns)
    {
        if column < 4 {
    ...
    let num_manipped_notes = unsorted_ticks
        .windows(2)
    ...
    score.longest_mcombo =
        util::longest_true_sequence(deviations.iter().map(|d| d.abs() <= 0.0225));
    ...
    score.wife2_wifescore = crate::Wife2::apply(
        &deviations,
    ...
    if let Some(timing_info) = timing_info_maybe {
        ...
        for (&note_second, &deviation, &column) in izip!(&note_seconds, &deviations, &columns) {
            if column >= 4 {
                continue;
            } // spooky scary ~~skeletons~~ >4k gamemodes
        ...
        let fastest_combo =
            find_fastest_combo_in_score(&note_seconds, &are_cbs, 100, 130, None, rate);
        let fastest_acc =
            find_fastest_combo_in_score(&note_seconds, &are_cbs, 100, 130, Some(&wife_pts), rate);
        ...
        score.timing_info_dependent_analysis = Some(TimingInfoDependentAnalysis {
            fastest_combo,
            fastest_acc,
            fastest_jack,
            new_wifescore,
...
fn calculate_standard_deviation(offset_buckets: &[u64], offset_bucket_range: u64) -> f32 {
    ...
    */
    ...
    // util function
    let iter_bucket_contents_and_sizes = || {
        offset_buckets
            .iter()
            .enumerate()
            .map(|(i, num_values_in_bucket)| {
                (i as i64 - offset_bucket_range as i64, num_values_in_bucket)
            })
    };
    ...
    squared_differences_mean.sqrt()
}

#[pymethods]
impl ReplaysAnalysis {
    /// Creates a new ReplaysAnalysis object.
    /// # Arguments
    /// * `prefix` - Path to the replays folder.
    /// * `scorekeys` - List of chartkeys.
    /// * `wifescores` - List of Wife3 scores.
    /// * `packs` - List of pack names.
    /// * `songs` - List of song names.
    /// * `rates` - List of score rates.
    /// * `songs_root` - Path to songs directory.
    #[new]
    pub fn create(
        prefix: &str,
        scorekeys: Vec<String>,
        wifescores: Vec<f32>,
        packs: Vec<String>,
        songs: Vec<String>,
        rates: Vec<f32>,
        ...
        let mut analysis = Self {
            offset_buckets: vec![0; NUM_OFFSET_BUCKETS as usize],
            sub_93_offset_buckets: vec![0; NUM_OFFSET_BUCKETS as usize],
            ..Default::default()
        };
        ...
        for (i, score_analysis_option) in score_analyses.into_iter().enumerate() {
            ...
            for (analysis_bucket, analysis_sub_93_bucket, score_bucket) in izip!(
                &mut analysis.offset_buckets,
                &mut analysis.sub_93_offset_buckets,
            ...
            if let Some(score_analysis) = score.timing_info_dependent_analysis {
                analysis.timing_info_dependent_score_indices.push(i as u64);
        ...
        assert_eq!(
            analysis.current_wifescores.len(),
            analysis.new_wifescores.len()
        );
        assert_eq!(
            analysis.current_wifescores.len(),
            analysis.timing_info_dependent_score_indices.len()
        );
        ...
        analysis.standard_deviation =
            calculate_standard_deviation(&analysis.offset_buckets, OFFSET_BUCKET_RANGE);
...
mod tests {
    ...
    fn test_find_fastest_note_subset() {
        ...
        fn test_the_functions(
            ...
            assert_eq!(
                fastest_subset.start_second,
                fastest_wife_pts_subset.start_second
            );
            assert_eq!(
                fastest_subset.end_second,
                fastest_wife_pts_subset.end_second
            );
        ...
        test_the_functions(seconds, 2, 99, 2, 0.66666666666); // should detect [3, 5, 6)
        test_the_functions(seconds, 3, 99, 3, 0.6); // should detect [3, 5, 6, 8)
        ...
        test_the_functions(seconds, 5, 6, 6, 1.5); // note that window size 6 is fastest! not 5
        ...
        test_the_functions(seconds, 5, 5, 5, 1.25);
    }
    ...
    fn test_calculate_standard_deviation() {
        ...
        let buckets = [
            7, 20, 32, 93, 178, 333, 517, 829, 1050, 1230, 1363, 1253, 1072, 807, 558, 319, 178,
            94, 43, 17, 4,
        ];
