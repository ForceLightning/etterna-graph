...
def try_select_xml() -> str | None:
    result = QFileDialog.getOpenFileName(
        caption="Select your Etterna.xml", filter="Etterna XML files(Etterna.xml)"
    )
    return result[0] if result else None


def try_choose_replays() -> str | None:
    _ = QMessageBox.information(None, "How to use", REPLAYS_CHOOSER_INFO_MSG)
    return QFileDialog.getExistingDirectory(caption="Select the ReplaysV2 directory")


def try_choose_songs_root() -> str | None:
    _ = QMessageBox.information(None, "How to use", SONGS_ROOT_CHOOSER_INFO_MSG)
    return QFileDialog.getExistingDirectory(caption="Select the root songs folder")
...
class SettingsEntry:
    ...
    chooser_fn: Any | None = None  # only applies to file/folder settings types
    min_max_values: tuple[int, int] | None = None  # only applies to spinbox
...
class Settings:
    def __init__(self) -> None:
        ...
        self.replays_dir: str | None
        self.songs_root: str | None
        self.enable_all_plots: bool
        self.hide_invalidated: bool
        self.bg_color: str
        self.text_color: str
...
class ColorPickerButton(QPushButton):
    ...
    def _update_self_color(self):
        self.setStyleSheet(
            f"background-color: {self._qcolordialog.currentColor().name()}"
        )
...
class SettingsDialog(QDialog):
    def __init__(self):
        ...
        _ = save_btn.pressed.connect(self.try_save)
        cancel_btn = buttons.addButton("Cancel", QDialogButtonBox.ButtonRole.RejectRole)
        _ = cancel_btn.pressed.connect(self.reject)
        ...
        for entry in SETTINGS_ENTRIES:
            ...
            if entry.settings_type in (SettingsType.File, SettingsType.Folder):
                ...
                def chooser_handler():
                    ...
                    if result:
                        self.input_widget.setText(result)
