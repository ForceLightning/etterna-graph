...
def show_score_info(xml: ET.Element, score: ET.Element) -> None:
    ...
    def show_all():
        ...
        lines.append(
            "- <b>"
            + " / ".join(tap_note_scores.findtext(name) for name in names)
            + "</b>"
        )
        ...
        try:
            ...
            mine_avoided_ratio = (
                1  # when there's no mines at all, just say the player has hit 100%
            )
        lines.append(
            f"- Avoided <b>{avoid_mine}/{hit_mine + avoid_mine}</b> mines (<b>{mine_avoided_ratio*100:.2f}%</b>)"
        )
        ...
        try:
            ...
            held_ratio = 1
        lines.append(
            f"- Held <b>{held}/{total_holds}</b> holds (<b>{held_ratio*100:.2f}%</b>)"
        )

        skillset_ssrs = score.find("SkillsetSSRs")
        if skillset_ssrs:
            lines.append(
                f"- Overall score rating: <b>{skillset_ssrs.findtext('Overall')}</b>"
            )
            for skillset_elem in skillset_ssrs:
                if skillset_elem.tag == "Overall":
                    continue  # we already showed that
...
def draw(
    ...
    try:  # First try UTF-8
        ...
        util.logger.exception(
            f"XML parsing with UTF-8 failed, falling back to {os_encoding}"
        )
        xmltree: ET.ElementTree = ET.parse(
            prefs.xml_path, ET.XMLParser(encoding=os_encoding)
        )
    xml: ET.Element = xmltree.getroot()
    ...
    def textbox(
        ...
        fn: Callable[[Any], Any],
        *args,
        read_more_title: str = None,
        link_handler: Callable[[Any], Any] = lambda link: None,
        **kwargs,
    ):
        ...
        def link_callback(link):
            if link == "#read_more" and read_more_title:
                show_scrollable_msgbox(
                    (fn)(*args, **kwargs, limit=None), read_more_title, word_wrap=True
                )
            else:
                (link_handler)(link)
    ...
    textbox(
        ...
        4,
        5,
        g.gen_text_most_played_charts,
        xml,
        read_more_title="Most played charts",
    )

    def most_played_packs_textbox(all_time: bool = False):  # spaghetti
        read_more_title = "Most played packs (" + (
            "all time)" if all_time else "last 6 months)"
        )
        textbox(
            4,
            0,
            8,
            3,
            g.gen_text_most_played_packs,
            xml,
            months=None if all_time else 6,
    ...
    textbox(
        0, 7, 4, 5, g.gen_text_longest_sessions, xml, read_more_title="Longest sessions"
    )
    ...
    def plotbox(plot, title: str, colspan: int = 1):
        ...
        label = QLabel(title)
        label.setStyleSheet(
            "font-size: 18px; font-weight: bold; border: 0px solid transparent"
        )
        label.setWordWrap(True)
    ...
    if prefs.enable_all_plots:
        ...
        plot = chart_wrapper.draw(
            ...
            color=cmap[0],
            data=(
                (analysis.current_wifescores, analysis.new_wifescores),
                analysis.wifescore_scores,
            ),
            click_callback=score_info_callback,
        )
        ...
        plot = chart_wrapper.draw(
            ...
            data=(
                (
                    analysis.wife2_wifescores,
                    [
                        float(score.findtext("SSRNormPercent"))
